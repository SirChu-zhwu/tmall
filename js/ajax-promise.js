...
function Ajax(options) {
    ...
    var obj = {
        ...
        async: true, //是否异步
        data: '', //请求参数
        datatype: 'string', //返回的数据格式(string:字符串形式返回,json:json数组形式返回)
    ...
    if (obj.type.toLowerCase() == "get") {
        //配置请求信息(加时间戳避免缓存)
        xhr.open("get", obj.url + (obj.data ? `?${obj.data}&_=${new Date().getTime()}` : ''), obj.async)
        //发送请求
        xhr.send()
        ...
        if (obj.data) {
            ...
            //配置请求信息(加时间戳避免缓存)
            xhr.send(`${obj.data}&_=${new Date().getTime()}`)
        } else {
            xhr.send()
        }
    }
